...
class Solution:
    ...
    def dfs(self, row, col, word):
        if word == "":
            ...
            for direction in directions:
                if self.dfs(row + direction[0], col + direction[1], word[1:]): return True
    ...
    def exist(self, board: List[List[str]], word: str) -> bool:
        ...
        for c in word: wordDict[c] += 1
        
        for c in string.ascii_letters:
            if wordDict[c] > boardDict[c]: return False
        
        if wordDict[word[0]] > wordDict[word[-1]]: word = word[::-1]

        self.board = [['0' for i in range(n + 2)]]
        for row in board: self.board.append(["0"] + row + ["0"])
