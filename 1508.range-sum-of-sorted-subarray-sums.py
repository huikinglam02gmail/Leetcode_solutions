    ...
    def rangeSum(self, nums: List[int], n: int, left: int, right: int) -> int:
        ...
        while ind <= right:
            ...
            if ind >= left:
                result += num
                result %= MOD
            if i < n - 1: heapq.heappush(heap, [nums[i + 1] + num, i + 1])
            ind += 1
        return result
...
class Solution:
    ...
    def countSubArraySumBelowS(self, S):
        ...
        for j in range(self.n + 1):
            while self.prefix[j] - self.prefix[i] > S: i += 1
            count += j - i
        return count
    
    def subarraySumGivenIndex(self, ind):
        l, r = 0, self.prefix[-1]
        while l < r:
            mid = (l + r) // 2
            if self.countSubArraySumBelowS(mid) < ind: l = mid + 1
            else: r = mid
        return l
    
    def sumAllSubArraysUpToIndex(self, ind):
        S = self.subarraySumGivenIndex(ind)
        result, i = 0, 0
        for j in range(self.n + 1):
            while self.prefix[j] - self.prefix[i] > S: i += 1
            result += self.prefix[j] * (j - i + 1) - self.prePrefix[j]
            if i > 0: result += self.prePrefix[i - 1]
