...
class Solution:
    ...
    def minimumDeletionsBinarySearch(self, s: str) -> int:
        ...
        hashTable['a'] = []
        hashTable['b'] = []
        for i, c in enumerate(s): hashTable[c].append(i)
        
        result = n
        for i in range(n): result = min(result, bisect.bisect_right(hashTable['b'], i - 1) + len(hashTable['a']) - bisect.bisect_left(hashTable['a'], i + 1))
        return result
